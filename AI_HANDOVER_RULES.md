...
| 2026-09-06 | **Perombakan Total Halaman Mutasi Saldo: Running Balance ('Sisa: Rp ...'), Ringkasan Finansial, Filter Kapsul Cepat, & Modal Detail Interaktif** | 1) Menghilangkan kelemahan mutasi saldo lama yang hanya menampilkan angka ambigu (seperti `-211` akibat order uncompleted tanpa konteks); 2) Membangun backend service `app/services/mutasi_service.py` untuk mengolah pergerakan saldo: memisahkan transaksi pembelian saldo (`-`), topup saldo berhasil (`+`), auto-refund transaksi gagal (`+`), dan klaim saldo poin (`+`); 3) Mengkalkulasi *running balance* matematis mundur dari saldo riil pengguna sehingga setiap mutasi menampilkan saldo akhir ("Sisa: Rp ...") persis seperti referensi antarmuka pengguna; 4) Mendaftarkan alias rute `@user_bp.route('/mutasi')` dan `@user_bp.route('/mutations')` berdampingan dengan `/informasi`, serta memperbarui ikon dompet aktif di bottom navbar `base.html`; 5) Merombak total template `app/templates/user/informasi.html` dengan desain modern: kartu saldo utama & quick topup, 3 kartu ringkasan finansial (Pemasukan, Pengeluaran, Refund), filter kapsul real-time, pencarian instan, dan bottom-sheet modal detail mutasi dengan opsi salin Reff ID dan pintasan invoice; 6) Verifikasi sistem 100% lulus tanpa regresi. | ✅ SELESAI (UI/UX & FINANSIAL) |
| 2026-09-06 | **Integrasi Penuh WhatsApp Bot Baileys ke Installer VPS (`install.sh`), CLI `garudatell`, & Arsitektur Pairing Code Stabil** | 1) Mengadopsi arsitektur pairing code Baileys 1:1 dari `bot-kasir`: konfigurasi `Browsers.ubuntu('Chrome')`, `markOnlineOnConnect: true`, `printQRInTerminal: false`, dan soket delay 3000ms sebelum `requestPairingCode` untuk stabilitas koneksi; 2) Menambahkan integrasi penuh WhatsApp Bot ke skrip instalasi VPS `install.sh`: auto-install Node.js 20 LTS, PM2 global, auto npm install dependensi `wa_bot`, start PM2 `garudatel-wa-bot`, dan registrasi auto-startup sistem `pm2 startup` & `pm2 save` sehingga bot langsung aktif sejak pertama kali VPS diinstal dan otomatis hidup kembali saat VPS reboot; 3) Memperbarui antarmuka CLI `garudatell`: penambahan status live WhatsApp Bot di header, penambahan menu `[5] Manajemen Bot WhatsApp (PM2)`, dan auto-update dependensi serta auto-restart PM2 saat memilih menu `[9] Cek Pembaruan (Git Update)`; 4) Menambahkan listener interaktif Telegram Bot Admin untuk callback `cmd_pair_new` dan balasan nomor `62` untuk penautan bot langsung via Telegram chat; 5) Memperbarui `README.md` dengan dokumentasi menu baru dan alur instalasi. | ✅ SELESAI (INTEGRASI & FITUR) |
| 2026-09-06 | **Perbaikan WhatsApp Bot Pairing (PM2 Auto-Recovery) & Alur Bantuan OTP Darurat 10 Menit via Bot Telegram CS** | 1) Mengatasi error 'pm2 blm di aktifkan' saat admin klik pairing dengan auto-recovery pada `app/routes/admin.py` (`_ensure_wa_bot_running`), endpoint baru `/admin/wa_restart` dan `/admin/wa_reset_session`, serta penambahan tombol 'Nyalakan / Restart Mesin' dan 'Reset Sesi' di panel admin; 2) Memperbaiki script `wa_bot/setup_pm2.sh` dan stabilitas `wa_bot/server_bot.js` dengan path absolut, global uncaught error handler, dan endpoint `/api/reset`; 3) Menghadirkan alur Bantuan OTP Darurat (Manual) terintegrasi: jika user gagal menerima OTP, tombol 'Bantuan OTP Darurat' akan membuat OTP khusus 10 menit (600 detik) di database dan mengirimkan notifikasi ke Bot Telegram CS (`BOT_CS_TOKEN`, `BOT_CS_CHAT_ID`) dengan direct link `https://wa.me/{nomor}?text=...` yang saat diklik langsung menampilkan pesan berisikan kode OTP ke WhatsApp user beserta informasi kode hanya berlaku 10 menit khusus di nomor tersebut; 4) Menyesuaikan panel Admin Dashboard Bantuan OTP Darurat dengan batas 10 menit, tombol buka WhatsApp direct, tombol salin kode OTP, dan status pengiriman ke Telegram CS; 5) Menambahkan unit test suite `tools/test_emergency_otp_flow.py` (4/4 test lulus) dan verifikasi seluruh sistem `test_full_system.py` (7/7 test lulus 100%). | ✅ SELESAI (BUGFIX & FITUR) |
